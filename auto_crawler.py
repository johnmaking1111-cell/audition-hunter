...
print(f"[*] Supabase DB 연결 타깃: {SUPABASE_URL}")
...
def parse_with_gemini(title: str, body: str) -> dict:
    ...
    {{
      ...
      "gender": "남, 여, 무관 중 택1",
      "age": "모집 연령 (예: 20대, 20대~30대, 전 연령)",
      "role": "배역명 또는 모집 요강 요약",
      "deadline": "마감일(YYYY-MM-DD 포맷, 기한 없거나 상시모집 시 2099-12-31)",
...
def run():
    ...
    # 해외 IP 차단 없이 실시간으로 배우/오디션/뮤지컬/캐스팅 공고가 수신되는 오픈 채널들
    feeds = [
    ...
    for source_name, feed_url in feeds:
        ...
        for post in posts[:4]:  # 각 채널당 상위 4개씩 즉각 정제
            source_url = post["link"]
            title = post["title"]
            desc = re.sub(r'<[^>]+>', '', post["desc"])  # HTML 태그 제거

            # 중복 검사
            exists = supabase.table("auditions").select("id").eq("source_url", source_url).execute()
            if exists.data:
                print(f"[=] 기존 적재 데이터 스킵: {title[:20]}...")
                continue
